  ...
  if (stage === 'idle') {
    return (
      <button
        ...
        className={\lex items-center gap-2 font-bold rounded-2xl transition-all
  ...
  return (
    <div className="bg-gradient-to-br from-red-50/80 to-rose-50/80 border border-red-200/60 rounded-2xl p-4 space-y-3">
      ...
      <input
        ...
        placeholder={\Digite: \\}
        autoFocus
        className="w-full h-10 px-3 rounded-xl border border-red-300/60 bg-white/90 text-red-900 font-medium text-sm
                   placeholder:text-red-600/50 focus:outline-none focus:ring-2 focus:ring-red-600"
      />

      <div className="flex gap-2 justify-end">
        <button
          onClick={() => { setStage('idle'); setInputValue(''); }}
          className="px-3 py-1.5 rounded-xl font-bold bg-gray-300 hover:bg-gray-400 text-gray-800 text-sm"
        >
          Cancelar
        </button>
        <button
          onClick={handleDelete}
          disabled={inputValue.toLowerCase() !== confirmText.toLowerCase() || loading}
          className="px-3 py-1.5 rounded-xl font-bold bg-gradient-to-r from-red-600 to-rose-600 
                     hover:from-red-700 hover:to-rose-700 disabled:from-gray-400 disabled:to-gray-400
                     text-white text-sm disabled:cursor-not-allowed"
        >
